...
<?php
    if ( isset($_POST['website_name']) && isset($_POST['website_url']) && isset($_POST['website_description']) ) {
        ...
        if ( isset($_FILES['website_img']) && $_FILES['website_img'] != "" && $_FILES['website_img']['error'] == 0 ) {
            /*
            ...
            $website->msh_SetWebsite($_GET['id'], $_POST['website_name'], $_POST['website_url'], $_POST['website_description'], $website_img, $_POST['website_category']);
        } else {
            require_once('../classes/Websites.class.php');
            $website = new Websites();
            $website_img = $website->msh_GetWebsiteById($_GET['id']);
            $website->msh_SetWebsite($_GET['id'], $_POST['website_name'], $_POST['website_url'], $_POST['website_description'], $website_img['website_img'], $_POST['website_category']);
        }
    ...
    require_once('../classes/Websites.class.php');
    $website = new Websites();
    $websiteInfo = $website->msh_GetWebsiteById($_GET['id']);
    $website_cat_name = $website->msh_GetWebsiteCategoryName($websiteInfo['website_cat']);
    
?>

<div id="main">
    <h1>Éditer le site : <?php echo stripslashes($websiteInfo['website_name']); ?></h1>
    <div id="content">
        <form method="POST" enctype="multipart/form-data">
            <label for="website_name">Nom du site :</label>
            <input type="text" id="website_name" name="website_name" placeholder="Ex: DuckDuckGo" maxlength="255" value="<?php echo stripslashes($websiteInfo['website_name']); ?>" />
            <label for="website_url">Adresse du site :</label>
            <input type="url" id="website_url" name="website_url" placeholder="Ex: https://duckduckgo.com/" maxlength="255" value="<?php echo $websiteInfo['website_url']; ?>" />
            <label for="website_img">Visuel du site :</label>
            <input type="file" id="website_img" name="website_img" />
            <label for="website_description">Description du site :</label>
            <textarea id="website_description" name="website_description"><?php echo stripslashes($websiteInfo['website_description']); ?></textarea>
            <label>Catégorie du site :</label>
            <select id="website_category" name="website_category">
                <option value="<?php echo $websiteInfo['website_cat']; ?>"><?php echo $website_cat_name; ?></option>
